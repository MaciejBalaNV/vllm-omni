...
async def create_video(
    raw_request: Request,
    ctx: tuple[
        VideoGenerationRequest, OmniOpenAIServingVideo, str, ReferenceImage | None, ReferenceVideo | None
    ...
    task = asyncio.create_task(
        _run_video_generation_job(
            handler, request, ref.id, reference_image, reference_video, app_state=raw_request.app.state
        )
    )
    ...
    try:
        video_bytes, stage_durations, peak_memory_mb, _action = await asyncio.wait_for(
            handler.generate_video_bytes(
                request, request_id, reference_image=reference_image, reference_video=reference_video
            ),
