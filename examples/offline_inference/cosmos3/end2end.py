...
def _load_action_data(path_or_url: str) -> Any:
    """Resolve an action JSON (local path or URL) and return its contents as
    inline data for ``extra_args['action']``. The file is read here, on the
    client/caller side, so the engine never reads paths off its filesystem."""
...
def _build_prompt_and_extra(
    ...
    if action_mode is not None:
        ...
        if action_mode == "forward_dynamics":
            ...
            extra_args["action"] = _load_action_data(args.action_path)
