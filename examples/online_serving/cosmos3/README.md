...
Forward-dynamics scripts download both the source vision asset and the matching action JSON. The action data is read client-side and passed inline as `action` (a JSON array) inside `extra_params`, so the server never reads files from its own filesystem — this works identically on same-host and cross-host deployments.
