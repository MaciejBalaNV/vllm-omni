...
# uses the sync video endpoint (raw MP4 response). The action data is read
# client-side and sent inline via `action` (a JSON array) in extra_params, so
# the server never reads files from its own filesystem.
...
EXTRA_PARAMS="$(jq -nc \
  ...
  --argjson action "$(cat "${ACTION_PATH}")" \
  '{action_mode:"forward_dynamics", domain_name:$domain, raw_action_dim:$dim, action_chunk_size:$chunk, action:$action}')"
