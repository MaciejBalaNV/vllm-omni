...
def multiview_upload_dir(test_client, monkeypatch, tmp_path):
    ...
    directory.mkdir()
    mkstemp = api_server.tempfile.mkstemp
    monkeypatch.setattr(api_server.tempfile, "mkstemp", lambda **kwargs: mkstemp(dir=directory, **kwargs))
    return directory
...
def test_multiview_partial_persistence_is_cleaned(bad_payload, test_client, multiview_upload_dir, monkeypatch):
    monkeypatch.setattr(api_server, "CONTROL_REFERENCE_MAX_BYTES", 8)
    extra, files = _multiview_upload_request()
